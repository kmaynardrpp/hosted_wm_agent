...
def build_user_message(user_prompt: str, csv_paths: List[str], project_dir: Path, trimmed: bool=False) -> str:
    ...
    INSTR = (
        ...
        "\n"
        "  # SAFE floor crop (GC): keep x>=12000 & y>=15000; handle empty/missing columns without errors\n"
...
- Per-file processing; GC crop (use safe floor crop x>=12000 & y>=15000); use dt.floor("h"); tables only if explicitly requested.
        ...
        for variant, msg in [("full", user_full), ("trimmed", user_trimmed)]:
            try:
                ...
                repair_prompt = (
                    ...
                    + "- Include MAC-map audit guard + smoke log; GC safe floor crop (x>=12000 & y>=15000) using _safe_floor_crop; use ts_utc; dt.floor('h').\n"
    ...
    for attempt in range(1, COMPILE_RETRIES + 1):
        try:
            ...
            repair_prompt = (
                ...
                "  • Keep DB auto-select (hyphen), inclusive ranges, ASSUME 2025; MAC-map guard; GC safe floor crop via _safe_floor_crop\n"
...
def runtime_repair_loop(client: OpenAI, model: str, system_msg: str, user_msg_full: str,
    ...
    for attempt in range(1, RUNTIME_RETRIES + 1):
        ...
        repair_msg = (
            ...
            + "- Keep DB auto-select (hyphen), inclusive ranges, assume 2025; MAC-map guard; GC safe floor crop; safe _print_links.\n"
